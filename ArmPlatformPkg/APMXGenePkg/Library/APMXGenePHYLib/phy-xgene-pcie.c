...
static int xgene_phy_pcie_wait_pll_lock(struct xgene_phy_ctx *ctx)
{
	...
	for (i = 0; i < loop; i++) {
		...
		do {
			if (i == 0)
				dt = readl(base + SDS0_CMU_STATUS0);
			else
				dt = readl(base + SDS1_CMU_STATUS0);
			if ((dt & 0x7) == 0x7)
				break;
			usleep_range(10, 100);
		} while (try--);
		if ((dt & 0x7) != 0x7) {
			dev_err(ctx->dev, "PHY PLL %d calibration %s\n", i,
				dt & 0x4 ? "successful" : "failed");
			dev_err(ctx->dev, "PHY PLL %d %slocked\n", i,
				dt & 0x2 ? "not " : "");
			dev_err(ctx->dev, "PHY PLL %d %sready\n", i,
				dt & 0x1 ? "not " : "");
			return -ENODEV;
		}
	}
...
static void xgene_phy_pcie_rcvr_detect(struct xgene_phy_ctx *ctx,
				       enum cmu_type_t cmu_type, int index)
{
	u32 dt;

	/* UP calibration */
	cmu_rd(ctx, cmu_type, CMU_REG17, &dt);
	dt = CMU_REG17_RESERVED_7_SET(dt, index);
	cmu_wr(ctx, cmu_type, CMU_REG17, dt);
	...
	cmu_rd(ctx, cmu_type, CMU_REG16, &dt);
	dt = CMU_REG16_UP_MAN_ENA_SET(dt, 0x1);
	cmu_wr(ctx, cmu_type, CMU_REG16, dt);

	cmu_rd(ctx, cmu_type, CMU_REG16, &dt);
	dt = CMU_REG16_UP_MAN_ENA_SET(dt, 0x0);
	cmu_wr(ctx, cmu_type, CMU_REG16, dt);
}
...
static void xgene_phy_pcie_manual_cal(struct xgene_phy_ctx *ctx)
{
	...
	for (j = 0; j < loop; j++) {
		for (i = 0; i < 4; i++) {
			...
			/* UP calibration */
			xgene_phy_pcie_rcvr_detect(ctx, PHY_CMU + j, i);
