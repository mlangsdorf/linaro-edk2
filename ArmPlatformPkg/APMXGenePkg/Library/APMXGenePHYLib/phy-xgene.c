...
#define DEFAULT_SATA_DFE                { 0x1, 0x1, 0x1 }
				  ...
				  enum clk_type_t clk_type, int ssc_enable)
{
	...
	if (ctx->inited == INITIALIZED) {
	   	for (lane = 0; lane < MAX_LANE; lane++) {
			serdes_rd(ctx, lane, RXTX_REG7, &val);
		    	speed = RXTX_REG7_RX_DATA_RATE_RD(val);
		    	switch(speed) {
			case 0:
...
static void xgene_phy_gen_avg_val(struct xgene_phy_ctx *ctx, int lane)
{
	...
	while (avg_loop < max_loop) {
		...
		if ((fail_even == 0 || fail_even == 1) &&
			...
			/* We don't report the first set of failure */
			dev_warn(ctx->dev,
				 "Lane %d receiver calibration failed at %d loop\n",
				 lane, avg_loop);
		}
